...
async function handleTCPOutBound(remoteSocket, addressRemote, portRemote, rawClientData, webSocket, vlessResponseHeader, log,) {
	...
	try {
		let finalAddress;
		// 1. 总是优先尝试将地址转换为 NAT64 地址
		try {
			log(`Attempting to get NAT64 address for ${addressRemote}`);
			finalAddress = await getDynamicProxyIP(addressRemote);
			log(`Successfully got NAT64 address: ${finalAddress}`);
		} catch (err) {
			// 2. 如果转换失败（例如纯IPv6主机），则回退到使用原始地址
			log(`Could not get NAT64 address. Falling back to direct connection. Error: ${err.message}`);
			finalAddress = addressRemote;
		}

		// 3. 使用最终确定的地址进行连接
		const tcpSocket = await connectAndWrite(finalAddress, portRemote);

		// 4. 将远程数据流对接到 WebSocket，不再需要 retry
		remoteSocketToWS(tcpSocket, webSocket, vlessResponseHeader, null, log);

	} catch (err) {
		// 这个 catch 块能捕获上面所有步骤中发生的致命错误（如连接失败）
		log(`Fatal: Connection logic failed for ${addressRemote}. Error: ${err.message}`);
